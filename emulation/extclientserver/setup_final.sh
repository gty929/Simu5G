...
sudo ip netns exec cns1 ip addr add 192.168.3.2/24 dev veth-client1
 
# Assign the address 192.168.3.2 with netmask 255.255.255.0 to `veth-nginx1`
sudo ip netns exec nns1 ip addr add 192.168.2.2/24 dev veth-nginx1
...
sudo ip netns exec nns1 nginx -c ~/go/src/sigs.k8s.io/scheduler-plugins/mynginx.conf
